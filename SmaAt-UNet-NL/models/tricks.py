...
class ASPP(nn.Module):
    def __init__(self, in_channel, depth=3) -> object:
        super(ASPP,self).__init__()
        self.mean = nn.AdaptiveAvgPool2d((1, 1))
        self.conv = nn.Conv2d(in_channel, depth, 1, 1)
        self.atrous_block1 = nn.Conv2d(in_channel, depth, 1, 1)

        self.atrous_block6 = nn.Conv2d(in_channel, depth, 3, 1, padding=1, dilation=1)
        self.atrous_block12 = nn.Conv2d(in_channel, depth, 3, 1, padding=2, dilation=2)
        self.atrous_block18 = nn.Conv2d(in_channel, depth, 3, 1, padding=3, dilation=3)
        self.conv_1x1_output = nn.Conv2d(depth * 5, depth, 1, 1)
        self.outconv=nn.Conv2d(depth,in_channel,kernel_size=1)
...
'''
Block Name: ASFF
Paper: https://arxiv.org/abs/1911.09516v1
'''
...
'''
Block Name: RFB
Paper: https://arxiv.org/abs/1711.07767
'''
...
if __name__ == '__main__':
    ...
    # net = NAFBlock(c=16)
    net = ASPP(in_channel=16)
    inp = torch.randn((4, 16, 9, 9))
    # net =ECANet(in_channels=12)
    y=net(inp)
    print(y.shape)
