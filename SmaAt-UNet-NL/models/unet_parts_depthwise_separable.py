...
class DoubleConvDS(nn.Module):#定义的深度可分离卷积
...
class DownFocusDS(nn.Module):
    """Downscaling with maxpool then double conv"""

    def __init__(self, in_channels, out_channels, kernels_per_layer=1):
        super().__init__()
        self.maxpool_conv = nn.Sequential(
            Focus(in_channels,in_channels),
            DoubleConvDS(in_channels, out_channels, kernels_per_layer=kernels_per_layer)
        )

    def forward(self, x):
        return self.maxpool_conv(x)
class DownDSDcn(nn.Module):
    """Downscaling with maxpool then double conv"""
